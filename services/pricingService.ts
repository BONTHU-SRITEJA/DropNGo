...
// ✅ Fetch current pricing (only pickup)
export const getCurrentPricing = async (): Promise<PricingConfig> => {
  const now = Date.now();
  if (cachedPricing && now - lastFetchTime < CACHE_DURATION) {
    return cachedPricing;
  ...
  try {
    ...
    // Merge DB values if present
    (pricingData || []).forEach((item: Pricing) => {
      if (item.service_type === 'pickup') {
        config.pickup[item.luggage_size] = item.price_per_hour;
      }
      if (item.base_pickup_fee) config.basePickupFee = item.base_pickup_fee;
      if (item.per_km_fee) config.perKmFee = item.per_km_fee;
    });
...
// ✅ Calculate booking price (pickup only)
export const calculateBookingPrice = (
  luggageItems: Array<{ size: string; quantity: number }>,
  storageHours: number,
  pricing: PricingConfig
): { storage: number; delivery: number; total: number } => {
  const storage = luggageItems.reduce((total, item) => {
    const sizeKey = item.size as keyof PricingConfig['pickup'];
    const pricePerHour = pricing.pickup[sizeKey] || 0;
    return total + pricePerHour * item.quantity * storageHours;
  }, 0);

  const delivery = pricing.basePickupFee;
